...
// Carga de plugs y webscraper Vespucio Norte
...
(async () => {
  ...
  await page.goto('https://www.vespucionorte.cl/Home/CirculasteSinTag#auto-consultas');
  ...
  await page.waitForSelector('#consulta-sintag');
  
  const consultas = await page.evaluate(() => {
    const rows = document.querySelectorAll('#consulta-sintag tbody tr');
    const consultas = [];
